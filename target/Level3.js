...
function addLevel3RaceFeatures(character) {
    if (character.subRaceName == "Drow Elf")
...
function handleSpellRechoosing(character, spellsKnown, spellList) {
    ...
    for (let x = 0; x < spellsKnown; x++) {
        ...
        character.memorizedSpells[level].push(util.choice(spellList[level], character.inherentSpells[level].concat(character.memorizedSpells[level])));
    }
}
...
function addClericLevel3Features(character) {
...
function addLevel3DomainSpells(character) {
...
function addFighterLevel3Features(character) {
...
function addSorcererLevel3Features(character) {
    ...
    for (let i = 0; i < 2; i++) {
        let level = util.choice([1, 2]);
        character.memorizedSpells[level].push(util.choice(sorcererSpells[level], character.inherentSpells[level].concat(character.memorizedSpells[level])));
    }
}
const metaMagic = [
...
function addWarlockLevel3Features(character) {
    ...
    for (let i = 0; i < 2; i++) {
        let level = util.choice([1, 2]);
        character.memorizedSpells[level].push(util.choice(warlockSpells[level], character.inherentSpells[level].concat(character.memorizedSpells[level])));
    }
}
function pactOfTheChain(character) {
    character.inherentSpells[1].push("Find familiar");
    return "Pact of the Chain";
}
function pactOfTheTome(character) {
    character.inherentSpells[0] = character.inherentSpells[0].concat(util.choices(warlockSpells[0], 3, character.inherentSpells[0].concat(character.memorizedSpells[0])));
    return "Pact of the Tome";
}
function addWizardLevel3Features(character) {
