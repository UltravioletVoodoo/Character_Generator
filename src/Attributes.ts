...
export function introduceStatBias(character: Character, attrs: Attributes): Attributes {
    if (character.className === "Barbarian")    attrs = makeHighest("str", attrs);
    if (character.className === "Bard")         attrs = makeHighest("cha", attrs);
    if (character.className === "Cleric")       attrs = makeHighest("wis", attrs);
    if (character.className === "Druid")        attrs = makeHighest("wis", attrs);
    if (character.className === "Fighter")      attrs = makeHighest(util.choice(["str", "dex"]), attrs);
    if (character.className === "Monk")         attrs = makeBothHighest("dex", "wis", attrs);
    if (character.className === "Paladin")      attrs = makeBothHighest("str", "cha", attrs);
    if (character.className === "Ranger")       attrs = makeBothHighest("dex", "wis", attrs);
    if (character.className === "Ranger*")      attrs = makeBothHighest("dex", "wis", attrs);
    if (character.className === "Rogue")        attrs = makeHighest("dex", attrs);
    if (character.className === "Sorcerer")     attrs = makeHighest("cha", attrs);
    if (character.className === "Warlock")      attrs = makeHighest("cha", attrs);
    if (character.className === "Wizard")       attrs = makeHighest("int", attrs);
...
function makeHighest(key: string, attrs: Attributes): Attributes {
    let keys: string[] = [
        "str",
        "dex",
        "con",
    ...
    let highest = "str";
    for (let x of keys) {
        if (attrs[x] > attrs[highest]) highest = x;
    }
    let temp = attrs[key];
...
function makeBothHighest(key1: string, key2: string, attrs: Attributes): Attributes {
    let keys: string[] = [
        "str",
        "dex",
        "con",
    ...
    for (let x of keys) {
        if (attrs[x] > attrs[secondHighest] && attrs[x] <= attrs[highest]) secondHighest = x;
    }
